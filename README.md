...
**Описание:**  
События используются для связи между слоями: моделью, представлением и презентером. Все они обрабатываются через брокер событий `EventEmitter`.

**Список событий:**

-   `card:change` – обновление списка карточек товаров
-   `card:selected` – выбор карточки товара (для открытия модального окна)
-   `card:basket` – добавление/удаление товара в корзине (при клике на кнопку карточки)
-   `preview:change` – обновление предпросмотра товара
-   `modal:open` – открытие модального окна
-   `modal:close` – закрытие модального окна
-   `basket:open` – открытие окна корзины
-   `basket:close` – закрытие окна корзины
-   `basket:change` – изменение содержимого корзины (обновление суммы, количества и списка)
-   `order:open` – переход к оформлению заказа
-   `order:change` – изменение данных заказа
-   `order:submit` – подтверждение первого шага (форма заказа)
-   `/^order\..*:changed/` – изменение конкретного поля заказа (валидность)
-   `contacts:submit` – отправка контактных данных (email и телефон)
